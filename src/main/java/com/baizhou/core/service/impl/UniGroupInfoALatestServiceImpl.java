...
@Service
public class UniGroupInfoALatestServiceImpl  implements UniGroupInfoALatestService {


    @Autowired
    private UniGroupInfoALatestRepo imp;
    /**
     * 保存
     ...
     */
    @Override
    public UniGroupInfoALatest saveUniGroupInfoALatest(UniGroupInfoALatestVO uniGroupInfoALatest) {

        return imp.save(BeanMapper.map(uniGroupInfoALatest, UniGroupInfoALatest.class));
    }
    /**
     * 删除单条记录
     * @param id
    ...
    public boolean deleteByKeyId(Long id) {
        UniGroupInfoALatest entity = imp.findById(id).orElse(null);
     ...
     */
    @Override
    public UniGroupInfoALatestVO findOneById(Long id) {
        UniGroupInfoALatest entity = imp.findById(id).orElse(null);
        if(entity == null) return null;
        return  BeanMapper.map(entity, UniGroupInfoALatestVO.class);
    }
    /**
    获取所有信息
     */
    @Override
    public List<UniGroupInfoALatestVO> getAllUniGroupInfoALatests() {
        return BeanMapper.mapList(imp.findAll(), UniGroupInfoALatestVO.class);
    }
    /**
     * 更新记录
     * @param uniGroupInfoALatestVO
     * @return
     */
    @Override
    public String updateUniGroupInfoALatest(UniGroupInfoALatestVO uniGroupInfoALatestVO) {
        UniGroupInfoALatest uniGroupInfoALatest = BeanMapper.map(uniGroupInfoALatestVO, UniGroupInfoALatest.class);
        imp.save(uniGroupInfoALatest);
        return "success";
    }
  ...
  public List<UniGroupInfoALatestVO> findByUniMajorCode(String uniMajorCode){
        List<UniGroupInfoALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoALatest> inList = imp.findByUniMajorCode(uniMajorCode);
        if(  inList != null){
            for (UniGroupInfoALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoALatestVO.class));
            }
        }
        return  voList;
  }
  ...
  public List<UniGroupInfoALatestVO> findByUniMajorCodeLike(String uniMajorCode){
        List<UniGroupInfoALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoALatest> inList = imp.findByUniMajorCodeLike("%"+uniMajorCode+"%");
        if(  inList != null){
            for (UniGroupInfoALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoALatestVO.class));
            }
  ...
  public List<UniGroupInfoALatestVO> findByPici(Integer pici){
        List<UniGroupInfoALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoALatest> inList = imp.findByPici(pici);
        if(  inList != null){
            for (UniGroupInfoALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoALatestVO.class));
            }
  ...
  public List<UniGroupInfoALatestVO> findBySchoolName(String schoolName){
        List<UniGroupInfoALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoALatest> inList = imp.findBySchoolName(schoolName);
        if(  inList != null){
            for (UniGroupInfoALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoALatestVO.class));
            }
  ...
  public List<UniGroupInfoALatestVO> findBySchoolNameLike(String schoolName){
        List<UniGroupInfoALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoALatest> inList = imp.findBySchoolNameLike("%"+schoolName+"%");
        if(  inList != null){
            for (UniGroupInfoALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoALatestVO.class));
            }
  ...
  public List<UniGroupInfoALatestVO> findBySchoolLvl1(Integer schoolLvl1){
        List<UniGroupInfoALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoALatest> inList = imp.findBySchoolLvl1(schoolLvl1);
        if(  inList != null){
            for (UniGroupInfoALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoALatestVO.class));
            }
  ...
  public List<UniGroupInfoALatestVO> findBySchoolLvl2(Integer schoolLvl2){
        List<UniGroupInfoALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoALatest> inList = imp.findBySchoolLvl2(schoolLvl2);
        if(  inList != null){
            for (UniGroupInfoALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoALatestVO.class));
            }
  ...
  public List<UniGroupInfoALatestVO> findBySchoolLvl3(Integer schoolLvl3){
        List<UniGroupInfoALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoALatest> inList = imp.findBySchoolLvl3(schoolLvl3);
        if(  inList != null){
            for (UniGroupInfoALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoALatestVO.class));
            }
  ...
  public List<UniGroupInfoALatestVO> findBySchoolType(String schoolType){
        List<UniGroupInfoALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoALatest> inList = imp.findBySchoolType(schoolType);
        if(  inList != null){
            for (UniGroupInfoALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoALatestVO.class));
            }
  ...
  public List<UniGroupInfoALatestVO> findBySchoolTypeLike(String schoolType){
        List<UniGroupInfoALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoALatest> inList = imp.findBySchoolTypeLike("%"+schoolType+"%");
        if(  inList != null){
            for (UniGroupInfoALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoALatestVO.class));
            }
  ...
  public List<UniGroupInfoALatestVO> findByProvince(String province){
        List<UniGroupInfoALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoALatest> inList = imp.findByProvince(province);
        if(  inList != null){
            for (UniGroupInfoALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoALatestVO.class));
            }
  ...
  public List<UniGroupInfoALatestVO> findByProvinceLike(String province){
        List<UniGroupInfoALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoALatest> inList = imp.findByProvinceLike("%"+province+"%");
        if(  inList != null){
            for (UniGroupInfoALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoALatestVO.class));
            }
  ...
  public List<UniGroupInfoALatestVO> findByExamYear(String examYear){
        List<UniGroupInfoALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoALatest> inList = imp.findByExamYear(examYear);
        if(  inList != null){
            for (UniGroupInfoALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoALatestVO.class));
            }
  ...
  public List<UniGroupInfoALatestVO> findByExamYearLike(String examYear){
        List<UniGroupInfoALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoALatest> inList = imp.findByExamYearLike("%"+examYear+"%");
        if(  inList != null){
            for (UniGroupInfoALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoALatestVO.class));
            }
  ...
  public List<UniGroupInfoALatestVO> findByMajorName(String majorName){
        List<UniGroupInfoALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoALatest> inList = imp.findByMajorName(majorName);
        if(  inList != null){
            for (UniGroupInfoALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoALatestVO.class));
            }
  ...
  public List<UniGroupInfoALatestVO> findByMajorNameLike(String majorName){
        List<UniGroupInfoALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoALatest> inList = imp.findByMajorNameLike("%"+majorName+"%");
        if(  inList != null){
            for (UniGroupInfoALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoALatestVO.class));
            }
...
//    public Map<String, Object> listbyPage(String uniMajorCode,Integer pici,String schoolName,Integer schoolLvl1,Integer schoolLvl2,Integer schoolLvl3,String schoolType,String province,String examYear,String majorName,Pageable pageable) {
    public Map<String, Object> listbyPage(Integer pici,  List<String> uniMajorCode,
        ...
        org.springframework.data.domain.Page<UniGroupInfoALatest> pageResult = null;
...
//        if (StringUtils.isEmpty(uniMajorCode)  && StringUtils.isEmpty(pici)  && StringUtils.isEmpty(schoolName)  && StringUtils.isEmpty(schoolLvl1)  && StringUtils.isEmpty(schoolLvl2)  && StringUtils.isEmpty(schoolLvl3)  && StringUtils.isEmpty(schoolType)  && StringUtils.isEmpty(province)  && StringUtils.isEmpty(majorName) ) {
            ...
            Specification<UniGroupInfoALatest> spec = (Specification<UniGroupInfoALatest>) (root, query, cb) -> {
...
//                    Predicate tempP1 = cb.and(cb.lessThanOrEqualTo(rank22, minRank), cb.greaterThan(rank22, 0)); //专业2
//                    Predicate tempP2 = cb.and(cb.lessThanOrEqualTo(major22, minRank), cb.greaterThan(major22, 0)); //专业2
//                    Predicate p = cb.or(tempP1, tempP2);
//                    predicates.add(p);
//
//                    Predicate tempP3 = cb.and(cb.greaterThanOrEqualTo(rank22, maxRank), cb.greaterThan(rank22, 0)); //专业2
//                    Predicate tempP4 = cb.and(cb.greaterThanOrEqualTo(major22, maxRank), cb.greaterThan(major22, 0)); //专业2
//                    Predicate p2 = cb.or(tempP3, tempP4);
//                    predicates.add(p2);

//                    Predicate p = cb.lessThanOrEqualTo(root.get("rankFor22"), minRank);
//                    p = cb.or(p, cb.lessThanOrEqualTo(root.get("major22LowRank"), minRank));
//                    predicates.add(p);
//
//
////                    predicates.add(cb.equal(root.get("major22LowRank"), maxRank));
//                    Predicate p2 = cb.greaterThanOrEqualTo(root.get("rankFor22"), maxRank);
//                    p2 = cb.or(p2, cb.greaterThanOrEqualTo(root.get("major22LowRank"), maxRank));
//                    predicates.add(p2);
...
//                    String[] majorNameSplits = majorName.split("\\|");
        ...
        List<UniGroupInfoALatestVO> list = BeanMapper.mapList(pageResult.getContent(), UniGroupInfoALatestVO.class);
//        List<com.msg.UniGroupInfoALatest> protoList = new ArrayList<>();
