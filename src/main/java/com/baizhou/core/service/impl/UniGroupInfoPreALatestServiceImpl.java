...
@Service
public class UniGroupInfoPreALatestServiceImpl  implements UniGroupInfoPreALatestService {


    @Autowired
    private UniGroupInfoPreALatestRepo imp;
    /**
     * 保存
     ...
     */
    @Override
    public UniGroupInfoPreALatest saveUniGroupInfoPreALatest(UniGroupInfoPreALatestVO uniGroupInfoPreALatest) {

        return imp.save(BeanMapper.map(uniGroupInfoPreALatest, UniGroupInfoPreALatest.class));
    }
    /**
     * 删除单条记录
     * @param id
    ...
    public boolean deleteByKeyId(Long id) {
        UniGroupInfoPreALatest entity = imp.findById(id).orElse(null);
     ...
     */
    @Override
    public UniGroupInfoPreALatestVO findOneById(Long id) {
        UniGroupInfoPreALatest entity = imp.findById(id).orElse(null);
        if(entity == null) return null;
        return  BeanMapper.map(entity, UniGroupInfoPreALatestVO.class);
    }
    /**
    获取所有信息
     */
    @Override
    public List<UniGroupInfoPreALatestVO> getAllUniGroupInfoPreALatests() {
        return BeanMapper.mapList(imp.findAll(), UniGroupInfoPreALatestVO.class);
    }
    /**
     * 更新记录
     * @param uniGroupInfoPreALatestVO
     * @return
     */
    @Override
    public String updateUniGroupInfoPreALatest(UniGroupInfoPreALatestVO uniGroupInfoPreALatestVO) {
        UniGroupInfoPreALatest uniGroupInfoPreALatest = BeanMapper.map(uniGroupInfoPreALatestVO, UniGroupInfoPreALatest.class);
        imp.save(uniGroupInfoPreALatest);
        return "success";
    }
  ...
  public List<UniGroupInfoPreALatestVO> findByUniMajorCode(String uniMajorCode){
        List<UniGroupInfoPreALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoPreALatest> inList = imp.findByUniMajorCode(uniMajorCode);
        if(  inList != null){
            for (UniGroupInfoPreALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoPreALatestVO.class));
            }
        }
        return  voList;
  }
  ...
  public List<UniGroupInfoPreALatestVO> findByUniMajorCodeLike(String uniMajorCode){
        List<UniGroupInfoPreALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoPreALatest> inList = imp.findByUniMajorCodeLike("%"+uniMajorCode+"%");
        if(  inList != null){
            for (UniGroupInfoPreALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoPreALatestVO.class));
            }
  ...
  public List<UniGroupInfoPreALatestVO> findByPici(Integer pici){
        List<UniGroupInfoPreALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoPreALatest> inList = imp.findByPici(pici);
        if(  inList != null){
            for (UniGroupInfoPreALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoPreALatestVO.class));
            }
  ...
  public List<UniGroupInfoPreALatestVO> findBySchoolName(String schoolName){
        List<UniGroupInfoPreALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoPreALatest> inList = imp.findBySchoolName(schoolName);
        if(  inList != null){
            for (UniGroupInfoPreALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoPreALatestVO.class));
            }
  ...
  public List<UniGroupInfoPreALatestVO> findBySchoolNameLike(String schoolName){
        List<UniGroupInfoPreALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoPreALatest> inList = imp.findBySchoolNameLike("%"+schoolName+"%");
        if(  inList != null){
            for (UniGroupInfoPreALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoPreALatestVO.class));
            }
  ...
  public List<UniGroupInfoPreALatestVO> findBySchoolLvl1(Integer schoolLvl1){
        List<UniGroupInfoPreALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoPreALatest> inList = imp.findBySchoolLvl1(schoolLvl1);
        if(  inList != null){
            for (UniGroupInfoPreALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoPreALatestVO.class));
            }
  ...
  public List<UniGroupInfoPreALatestVO> findBySchoolLvl2(Integer schoolLvl2){
        List<UniGroupInfoPreALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoPreALatest> inList = imp.findBySchoolLvl2(schoolLvl2);
        if(  inList != null){
            for (UniGroupInfoPreALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoPreALatestVO.class));
            }
  ...
  public List<UniGroupInfoPreALatestVO> findBySchoolLvl3(Integer schoolLvl3){
        List<UniGroupInfoPreALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoPreALatest> inList = imp.findBySchoolLvl3(schoolLvl3);
        if(  inList != null){
            for (UniGroupInfoPreALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoPreALatestVO.class));
            }
  ...
  public List<UniGroupInfoPreALatestVO> findBySchoolType(String schoolType){
        List<UniGroupInfoPreALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoPreALatest> inList = imp.findBySchoolType(schoolType);
        if(  inList != null){
            for (UniGroupInfoPreALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoPreALatestVO.class));
            }
  ...
  public List<UniGroupInfoPreALatestVO> findBySchoolTypeLike(String schoolType){
        List<UniGroupInfoPreALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoPreALatest> inList = imp.findBySchoolTypeLike("%"+schoolType+"%");
        if(  inList != null){
            for (UniGroupInfoPreALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoPreALatestVO.class));
            }
  ...
  public List<UniGroupInfoPreALatestVO> findByProvince(String province){
        List<UniGroupInfoPreALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoPreALatest> inList = imp.findByProvince(province);
        if(  inList != null){
            for (UniGroupInfoPreALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoPreALatestVO.class));
            }
  ...
  public List<UniGroupInfoPreALatestVO> findByProvinceLike(String province){
        List<UniGroupInfoPreALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoPreALatest> inList = imp.findByProvinceLike("%"+province+"%");
        if(  inList != null){
            for (UniGroupInfoPreALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoPreALatestVO.class));
            }
  ...
  public List<UniGroupInfoPreALatestVO> findByExamYear(String examYear){
        List<UniGroupInfoPreALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoPreALatest> inList = imp.findByExamYear(examYear);
        if(  inList != null){
            for (UniGroupInfoPreALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoPreALatestVO.class));
            }
  ...
  public List<UniGroupInfoPreALatestVO> findByExamYearLike(String examYear){
        List<UniGroupInfoPreALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoPreALatest> inList = imp.findByExamYearLike("%"+examYear+"%");
        if(  inList != null){
            for (UniGroupInfoPreALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoPreALatestVO.class));
            }
  ...
  public List<UniGroupInfoPreALatestVO> findByMajorName(String majorName){
        List<UniGroupInfoPreALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoPreALatest> inList = imp.findByMajorName(majorName);
        if(  inList != null){
            for (UniGroupInfoPreALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoPreALatestVO.class));
            }
  ...
  public List<UniGroupInfoPreALatestVO> findByMajorNameLike(String majorName){
        List<UniGroupInfoPreALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoPreALatest> inList = imp.findByMajorNameLike("%"+majorName+"%");
        if(  inList != null){
            for (UniGroupInfoPreALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoPreALatestVO.class));
            }
...
//    public Map<String, Object> listbyPage(String uniMajorCode,Integer pici,String schoolName,Integer schoolLvl1,Integer schoolLvl2,Integer schoolLvl3,String schoolType,String province,String examYear,String majorName,Pageable pageable) {
    public Map<String, Object> listbyPage(Integer pici,  List<String> uniMajorCode,
        ...
        org.springframework.data.domain.Page<UniGroupInfoPreALatest> pageResult = null;
...
//        if (StringUtils.isEmpty(uniMajorCode)  && StringUtils.isEmpty(pici)  && StringUtils.isEmpty(schoolName)  && StringUtils.isEmpty(schoolLvl1)  && StringUtils.isEmpty(schoolLvl2)  && StringUtils.isEmpty(schoolLvl3)  && StringUtils.isEmpty(schoolType)  && StringUtils.isEmpty(province)  && StringUtils.isEmpty(majorName) ) {
            ...
            Specification<UniGroupInfoPreALatest> spec = (Specification<UniGroupInfoPreALatest>) (root, query, cb) -> {
...
//                    String[] majorNameSplits = majorName.split("\\|");
        ...
        List<UniGroupInfoPreALatestVO> list = BeanMapper.mapList(pageResult.getContent(), UniGroupInfoPreALatestVO.class);
//        List<com.msg.UniGroupInfoPreALatest> protoList = new ArrayList<>();
