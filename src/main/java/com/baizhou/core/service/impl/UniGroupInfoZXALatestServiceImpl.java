...
@Service
public class UniGroupInfoZXALatestServiceImpl  implements UniGroupInfoZXALatestService {


    @Autowired
    private UniGroupInfoZXALatestRepo imp;
    /**
     * 保存
     ...
     */
    @Override
    public UniGroupInfoZXALatest saveUniGroupInfoZXALatest(UniGroupInfoZXALatestVO uniGroupInfoZXALatest) {

        return imp.save(BeanMapper.map(uniGroupInfoZXALatest, UniGroupInfoZXALatest.class));
    }
    /**
     * 删除单条记录
     * @param id
    ...
    public boolean deleteByKeyId(Long id) {
        UniGroupInfoZXALatest entity = imp.findById(id).orElse(null);
     ...
     */
    @Override
    public UniGroupInfoZXALatestVO findOneById(Long id) {
        UniGroupInfoZXALatest entity = imp.findById(id).orElse(null);
        if(entity == null) return null;
        return  BeanMapper.map(entity, UniGroupInfoZXALatestVO.class);
    }
    /**
    获取所有信息
     */
    @Override
    public List<UniGroupInfoZXALatestVO> getAllUniGroupInfoZXALatests() {
        return BeanMapper.mapList(imp.findAll(), UniGroupInfoZXALatestVO.class);
    }
    /**
     * 更新记录
     * @param uniGroupInfoZXALatestVO
     * @return
     */
    @Override
    public String updateUniGroupInfoZXALatest(UniGroupInfoZXALatestVO uniGroupInfoZXALatestVO) {
        UniGroupInfoZXALatest uniGroupInfoZXALatest = BeanMapper.map(uniGroupInfoZXALatestVO, UniGroupInfoZXALatest.class);
        imp.save(uniGroupInfoZXALatest);
        return "success";
    }
  ...
  public List<UniGroupInfoZXALatestVO> findByUniMajorCode(String uniMajorCode){
        List<UniGroupInfoZXALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZXALatest> inList = imp.findByUniMajorCode(uniMajorCode);
        if(  inList != null){
            for (UniGroupInfoZXALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZXALatestVO.class));
            }
        }
        return  voList;
  }
  ...
  public List<UniGroupInfoZXALatestVO> findByUniMajorCodeLike(String uniMajorCode){
        List<UniGroupInfoZXALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZXALatest> inList = imp.findByUniMajorCodeLike("%"+uniMajorCode+"%");
        if(  inList != null){
            for (UniGroupInfoZXALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZXALatestVO.class));
            }
  ...
  public List<UniGroupInfoZXALatestVO> findByPici(Integer pici){
        List<UniGroupInfoZXALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZXALatest> inList = imp.findByPici(pici);
        if(  inList != null){
            for (UniGroupInfoZXALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZXALatestVO.class));
            }
  ...
  public List<UniGroupInfoZXALatestVO> findBySchoolName(String schoolName){
        List<UniGroupInfoZXALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZXALatest> inList = imp.findBySchoolName(schoolName);
        if(  inList != null){
            for (UniGroupInfoZXALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZXALatestVO.class));
            }
  ...
  public List<UniGroupInfoZXALatestVO> findBySchoolNameLike(String schoolName){
        List<UniGroupInfoZXALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZXALatest> inList = imp.findBySchoolNameLike("%"+schoolName+"%");
        if(  inList != null){
            for (UniGroupInfoZXALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZXALatestVO.class));
            }
  ...
  public List<UniGroupInfoZXALatestVO> findBySchoolLvl1(Integer schoolLvl1){
        List<UniGroupInfoZXALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZXALatest> inList = imp.findBySchoolLvl1(schoolLvl1);
        if(  inList != null){
            for (UniGroupInfoZXALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZXALatestVO.class));
            }
  ...
  public List<UniGroupInfoZXALatestVO> findBySchoolLvl2(Integer schoolLvl2){
        List<UniGroupInfoZXALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZXALatest> inList = imp.findBySchoolLvl2(schoolLvl2);
        if(  inList != null){
            for (UniGroupInfoZXALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZXALatestVO.class));
            }
  ...
  public List<UniGroupInfoZXALatestVO> findBySchoolLvl3(Integer schoolLvl3){
        List<UniGroupInfoZXALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZXALatest> inList = imp.findBySchoolLvl3(schoolLvl3);
        if(  inList != null){
            for (UniGroupInfoZXALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZXALatestVO.class));
            }
  ...
  public List<UniGroupInfoZXALatestVO> findBySchoolType(String schoolType){
        List<UniGroupInfoZXALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZXALatest> inList = imp.findBySchoolType(schoolType);
        if(  inList != null){
            for (UniGroupInfoZXALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZXALatestVO.class));
            }
  ...
  public List<UniGroupInfoZXALatestVO> findBySchoolTypeLike(String schoolType){
        List<UniGroupInfoZXALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZXALatest> inList = imp.findBySchoolTypeLike("%"+schoolType+"%");
        if(  inList != null){
            for (UniGroupInfoZXALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZXALatestVO.class));
            }
  ...
  public List<UniGroupInfoZXALatestVO> findByProvince(String province){
        List<UniGroupInfoZXALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZXALatest> inList = imp.findByProvince(province);
        if(  inList != null){
            for (UniGroupInfoZXALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZXALatestVO.class));
            }
  ...
  public List<UniGroupInfoZXALatestVO> findByProvinceLike(String province){
        List<UniGroupInfoZXALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZXALatest> inList = imp.findByProvinceLike("%"+province+"%");
        if(  inList != null){
            for (UniGroupInfoZXALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZXALatestVO.class));
            }
  ...
  public List<UniGroupInfoZXALatestVO> findByExamYear(String examYear){
        List<UniGroupInfoZXALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZXALatest> inList = imp.findByExamYear(examYear);
        if(  inList != null){
            for (UniGroupInfoZXALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZXALatestVO.class));
            }
  ...
  public List<UniGroupInfoZXALatestVO> findByExamYearLike(String examYear){
        List<UniGroupInfoZXALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZXALatest> inList = imp.findByExamYearLike("%"+examYear+"%");
        if(  inList != null){
            for (UniGroupInfoZXALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZXALatestVO.class));
            }
  ...
  public List<UniGroupInfoZXALatestVO> findByMajorName(String majorName){
        List<UniGroupInfoZXALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZXALatest> inList = imp.findByMajorName(majorName);
        if(  inList != null){
            for (UniGroupInfoZXALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZXALatestVO.class));
            }
  ...
  public List<UniGroupInfoZXALatestVO> findByMajorNameLike(String majorName){
        List<UniGroupInfoZXALatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZXALatest> inList = imp.findByMajorNameLike("%"+majorName+"%");
        if(  inList != null){
            for (UniGroupInfoZXALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZXALatestVO.class));
            }
...
//    public Map<String, Object> listbyPage(String uniMajorCode,Integer pici,String schoolName,Integer schoolLvl1,Integer schoolLvl2,Integer schoolLvl3,String schoolType,String province,String examYear,String majorName,Pageable pageable) {
    public Map<String, Object> listbyPage(Integer pici,  List<String> uniMajorCode,
        ...
        org.springframework.data.domain.Page<UniGroupInfoZXALatest> pageResult = null;
...
//        if (StringUtils.isEmpty(uniMajorCode)  && StringUtils.isEmpty(pici)  && StringUtils.isEmpty(schoolName)  && StringUtils.isEmpty(schoolLvl1)  && StringUtils.isEmpty(schoolLvl2)  && StringUtils.isEmpty(schoolLvl3)  && StringUtils.isEmpty(schoolType)  && StringUtils.isEmpty(province)  && StringUtils.isEmpty(majorName) ) {
            ...
            Specification<UniGroupInfoZXALatest> spec = (Specification<UniGroupInfoZXALatest>) (root, query, cb) -> {
...
//                    String[] majorNameSplits = majorName.split("\\|");
        ...
        List<UniGroupInfoZXALatestVO> list = BeanMapper.mapList(pageResult.getContent(), UniGroupInfoZXALatestVO.class);
//        List<com.msg.UniGroupInfoZXALatest> protoList = new ArrayList<>();
