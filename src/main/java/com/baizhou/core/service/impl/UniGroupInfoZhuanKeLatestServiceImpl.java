...
@Service
public class UniGroupInfoZhuanKeLatestServiceImpl  implements UniGroupInfoZhuanKeLatestService {


    @Autowired
    private UniGroupInfoZhuanKeLatestRepo imp;
    /**
     * 保存
     ...
     */
    @Override
    public UniGroupInfoZhuanKeLatest saveUniGroupInfoZhuanKeLatest(UniGroupInfoZhuanKeLatestVO uniGroupInfoZhuanKeLatest) {

        return imp.save(BeanMapper.map(uniGroupInfoZhuanKeLatest, UniGroupInfoZhuanKeLatest.class));
    }
    /**
     * 删除单条记录
     * @param id
    ...
    public boolean deleteByKeyId(Long id) {
        UniGroupInfoZhuanKeLatest entity = imp.findById(id).orElse(null);
     ...
     */
    @Override
    public UniGroupInfoZhuanKeLatestVO findOneById(Long id) {
        UniGroupInfoZhuanKeLatest entity = imp.findById(id).orElse(null);
        if(entity == null) return null;
        return  BeanMapper.map(entity, UniGroupInfoZhuanKeLatestVO.class);
    }
    /**
    获取所有信息
     */
    @Override
    public List<UniGroupInfoZhuanKeLatestVO> getAllUniGroupInfoZhuanKeLatests() {
        return BeanMapper.mapList(imp.findAll(), UniGroupInfoZhuanKeLatestVO.class);
    }
    /**
     * 更新记录
     * @param uniGroupInfoZhuanKeLatestVO
     * @return
     */
    @Override
    public String updateUniGroupInfoZhuanKeLatest(UniGroupInfoZhuanKeLatestVO uniGroupInfoZhuanKeLatestVO) {
        UniGroupInfoZhuanKeLatest uniGroupInfoZhuanKeLatest = BeanMapper.map(uniGroupInfoZhuanKeLatestVO, UniGroupInfoZhuanKeLatest.class);
        imp.save(uniGroupInfoZhuanKeLatest);
        return "success";
    }
  ...
  public List<UniGroupInfoZhuanKeLatestVO> findByUniMajorCode(String uniMajorCode){
        List<UniGroupInfoZhuanKeLatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZhuanKeLatest> inList = imp.findByUniMajorCode(uniMajorCode);
        if(  inList != null){
            for (UniGroupInfoZhuanKeLatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZhuanKeLatestVO.class));
            }
        }
        return  voList;
  }
  ...
  public List<UniGroupInfoZhuanKeLatestVO> findByUniMajorCodeLike(String uniMajorCode){
        List<UniGroupInfoZhuanKeLatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZhuanKeLatest> inList = imp.findByUniMajorCodeLike("%"+uniMajorCode+"%");
        if(  inList != null){
            for (UniGroupInfoZhuanKeLatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZhuanKeLatestVO.class));
            }
  ...
  public List<UniGroupInfoZhuanKeLatestVO> findByPici(Integer pici){
        List<UniGroupInfoZhuanKeLatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZhuanKeLatest> inList = imp.findByPici(pici);
        if(  inList != null){
            for (UniGroupInfoZhuanKeLatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZhuanKeLatestVO.class));
            }
  ...
  public List<UniGroupInfoZhuanKeLatestVO> findBySchoolName(String schoolName){
        List<UniGroupInfoZhuanKeLatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZhuanKeLatest> inList = imp.findBySchoolName(schoolName);
        if(  inList != null){
            for (UniGroupInfoZhuanKeLatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZhuanKeLatestVO.class));
            }
  ...
  public List<UniGroupInfoZhuanKeLatestVO> findBySchoolNameLike(String schoolName){
        List<UniGroupInfoZhuanKeLatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZhuanKeLatest> inList = imp.findBySchoolNameLike("%"+schoolName+"%");
        if(  inList != null){
            for (UniGroupInfoZhuanKeLatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZhuanKeLatestVO.class));
            }
  ...
  public List<UniGroupInfoZhuanKeLatestVO> findBySchoolLvl1(Integer schoolLvl1){
        List<UniGroupInfoZhuanKeLatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZhuanKeLatest> inList = imp.findBySchoolLvl1(schoolLvl1);
        if(  inList != null){
            for (UniGroupInfoZhuanKeLatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZhuanKeLatestVO.class));
            }
  ...
  public List<UniGroupInfoZhuanKeLatestVO> findBySchoolLvl2(Integer schoolLvl2){
        List<UniGroupInfoZhuanKeLatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZhuanKeLatest> inList = imp.findBySchoolLvl2(schoolLvl2);
        if(  inList != null){
            for (UniGroupInfoZhuanKeLatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZhuanKeLatestVO.class));
            }
  ...
  public List<UniGroupInfoZhuanKeLatestVO> findBySchoolLvl3(Integer schoolLvl3){
        List<UniGroupInfoZhuanKeLatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZhuanKeLatest> inList = imp.findBySchoolLvl3(schoolLvl3);
        if(  inList != null){
            for (UniGroupInfoZhuanKeLatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZhuanKeLatestVO.class));
            }
  ...
  public List<UniGroupInfoZhuanKeLatestVO> findBySchoolType(String schoolType){
        List<UniGroupInfoZhuanKeLatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZhuanKeLatest> inList = imp.findBySchoolType(schoolType);
        if(  inList != null){
            for (UniGroupInfoZhuanKeLatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZhuanKeLatestVO.class));
            }
  ...
  public List<UniGroupInfoZhuanKeLatestVO> findBySchoolTypeLike(String schoolType){
        List<UniGroupInfoZhuanKeLatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZhuanKeLatest> inList = imp.findBySchoolTypeLike("%"+schoolType+"%");
        if(  inList != null){
            for (UniGroupInfoZhuanKeLatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZhuanKeLatestVO.class));
            }
  ...
  public List<UniGroupInfoZhuanKeLatestVO> findByProvince(String province){
        List<UniGroupInfoZhuanKeLatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZhuanKeLatest> inList = imp.findByProvince(province);
        if(  inList != null){
            for (UniGroupInfoZhuanKeLatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZhuanKeLatestVO.class));
            }
  ...
  public List<UniGroupInfoZhuanKeLatestVO> findByProvinceLike(String province){
        List<UniGroupInfoZhuanKeLatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZhuanKeLatest> inList = imp.findByProvinceLike("%"+province+"%");
        if(  inList != null){
            for (UniGroupInfoZhuanKeLatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZhuanKeLatestVO.class));
            }
  ...
  public List<UniGroupInfoZhuanKeLatestVO> findByExamYear(String examYear){
        List<UniGroupInfoZhuanKeLatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZhuanKeLatest> inList = imp.findByExamYear(examYear);
        if(  inList != null){
            for (UniGroupInfoZhuanKeLatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZhuanKeLatestVO.class));
            }
  ...
  public List<UniGroupInfoZhuanKeLatestVO> findByExamYearLike(String examYear){
        List<UniGroupInfoZhuanKeLatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZhuanKeLatest> inList = imp.findByExamYearLike("%"+examYear+"%");
        if(  inList != null){
            for (UniGroupInfoZhuanKeLatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZhuanKeLatestVO.class));
            }
  ...
  public List<UniGroupInfoZhuanKeLatestVO> findByMajorName(String majorName){
        List<UniGroupInfoZhuanKeLatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZhuanKeLatest> inList = imp.findByMajorName(majorName);
        if(  inList != null){
            for (UniGroupInfoZhuanKeLatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZhuanKeLatestVO.class));
            }
  ...
  public List<UniGroupInfoZhuanKeLatestVO> findByMajorNameLike(String majorName){
        List<UniGroupInfoZhuanKeLatestVO> voList = new ArrayList<>();
        List<UniGroupInfoZhuanKeLatest> inList = imp.findByMajorNameLike("%"+majorName+"%");
        if(  inList != null){
            for (UniGroupInfoZhuanKeLatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniGroupInfoZhuanKeLatestVO.class));
            }
...
//    public Map<String, Object> listbyPage(String uniMajorCode,Integer pici,String schoolName,Integer schoolLvl1,Integer schoolLvl2,Integer schoolLvl3,String schoolType,String province,String examYear,String majorName,Pageable pageable) {
    public Map<String, Object> listbyPage(Integer pici,  List<String> uniMajorCode,
        ...
        org.springframework.data.domain.Page<UniGroupInfoZhuanKeLatest> pageResult = null;
...
//        if (StringUtils.isEmpty(uniMajorCode)  && StringUtils.isEmpty(pici)  && StringUtils.isEmpty(schoolName)  && StringUtils.isEmpty(schoolLvl1)  && StringUtils.isEmpty(schoolLvl2)  && StringUtils.isEmpty(schoolLvl3)  && StringUtils.isEmpty(schoolType)  && StringUtils.isEmpty(province)  && StringUtils.isEmpty(majorName) ) {
            ...
            Specification<UniGroupInfoZhuanKeLatest> spec = (Specification<UniGroupInfoZhuanKeLatest>) (root, query, cb) -> {
...
//                    String[] majorNameSplits = majorName.split("\\|");
        ...
        List<UniGroupInfoZhuanKeLatestVO> list = BeanMapper.mapList(pageResult.getContent(), UniGroupInfoZhuanKeLatestVO.class);
//        List<com.msg.UniGroupInfoZhuanKeLatest> protoList = new ArrayList<>();
