...
@Service
public class UniMajorInfoALatestServiceImpl implements UniMajorInfoALatestService {


    @Autowired
    private UniMajorInfoALatestRepo imp;
     ...
     */
    @Override
    public UniMajorInfoALatest saveUniMajorInfoALatest(UniMajorInfoALatestVO uniMajorInfoALatest) {

        return imp.save(BeanMapper.map(uniMajorInfoALatest, UniMajorInfoALatest.class));
    }
    ...
    public boolean deleteByKeyId(Long id) {
        UniMajorInfoALatest entity = imp.findById(id).orElse(null);
     ...
     */
    @Override
    public UniMajorInfoALatestVO findOneById(Long id) {
        UniMajorInfoALatest entity = imp.findById(id).orElse(null);
        if (entity == null) return null;
        return BeanMapper.map(entity, UniMajorInfoALatestVO.class);
    }

    @Override
    public List<UniMajorInfoALatestVO> findByIdIn(List<Long> id) {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findByIdIn(id);
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
        }
        return voList;
    }
     ...
     */
    @Override
    public List<UniMajorInfoALatestVO> getAllUniMajorInfoALatests() {
        return BeanMapper.mapList(imp.findAll(), UniMajorInfoALatestVO.class);
    }
     ...
     */
    @Override
    public String updateUniMajorInfoALatest(UniMajorInfoALatestVO uniMajorInfoALatestVO) {
        UniMajorInfoALatest uniMajorInfoALatest = BeanMapper.map(uniMajorInfoALatestVO, UniMajorInfoALatest.class);
    ...
    public List<UniMajorInfoALatestVO> findByUniMajorCode(String uniMajorCode) {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findByUniMajorCode(uniMajorCode);
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    public List<UniMajorInfoALatestVO> findByUniMajorCodeLike(String uniMajorCode) {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findByUniMajorCodeLike("%" + uniMajorCode + "%");
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    public List<UniMajorInfoALatestVO> findByPici(Integer pici) {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findByPici(pici);
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    public List<UniMajorInfoALatestVO> findBySchoolName(String schoolName) {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findBySchoolName(schoolName);
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    public List<UniMajorInfoALatestVO> findBySchoolNameLike(String schoolName) {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findBySchoolNameLike("%" + schoolName + "%");
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    public List<UniMajorInfoALatestVO> findBySchoolLvl1(Integer schoolLvl1) {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findBySchoolLvl1(schoolLvl1);
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    public List<UniMajorInfoALatestVO> findBySchoolLvl2(Integer schoolLvl2) {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findBySchoolLvl2(schoolLvl2);
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    public List<UniMajorInfoALatestVO> findBySchoolLvl3(Integer schoolLvl3) {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findBySchoolLvl3(schoolLvl3);
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    public List<UniMajorInfoALatestVO> findBySchoolType(String schoolType) {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findBySchoolType(schoolType);
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    public List<UniMajorInfoALatestVO> findBySchoolTypeLike(String schoolType) {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findBySchoolTypeLike("%" + schoolType + "%");
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    public List<UniMajorInfoALatestVO> findByProvince(String province) {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findByProvince(province);
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    public List<UniMajorInfoALatestVO> findByProvinceLike(String province) {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findByProvinceLike("%" + province + "%");
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    public List<UniMajorInfoALatestVO> findByMajorName(String majorName) {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findByMajorName(majorName);
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    public List<UniMajorInfoALatestVO> findByMajorNameLike(String majorName) {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findByMajorNameLike("%" + majorName + "%");
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    public List<UniMajorInfoALatestVO> findByIsZhongWai(Integer isZhongWai){
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findByIsZhongWai(isZhongWai);
        if(  inList != null){
            for (UniMajorInfoALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoALatestVO.class));
            }
    ...
    public List<UniMajorInfoALatestVO> findByIsBenSuo(Integer isBenSuo){
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findByIsBenSuo(isBenSuo);
        if(  inList != null){
            for (UniMajorInfoALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoALatestVO.class));
            }
...
//    public Map<String, Object> listbyPage(String uniMajorCode,Integer pici,String schoolName,Integer schoolLvl1,Integer schoolLvl2,Integer schoolLvl3,String schoolType,String province,String majorName,Pageable pageable) {
        ...
        org.springframework.data.domain.Page<UniMajorInfoALatest> pageResult = null;
...
//            boolean finalIsMajor2Code = isMajor2Code;
            ...
            Specification<UniMajorInfoALatest> spec = (Specification<UniMajorInfoALatest>) (root, query, cb) -> {
...
//            pageResult = imp.findPage(spec, pageable);
        ...
        List<UniMajorInfoALatestVO> list = BeanMapper.mapList(pageResult.getContent(), UniMajorInfoALatestVO.class);
...
//        }
    ...
    @Override
    public List<UniMajorInfoALatestVO> findByUniMajorCodeIn(List<String> ids) {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findByUniMajorCodeIn(ids);
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoALatestVO> findByUniMajorCodeAndExamYear(String uniMajorCode, String examYear) {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findByUniMajorCodeAndExamYear(uniMajorCode, examYear);
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoALatestVO> findByHistMajorIdsAndExamYear(List<Long> histDBIds, String examYear)
    {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findByHistMajorIdsAndExamYear( histDBIds, examYear);
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoALatestVO> findByUniMajorCodeAndMajorCodeAndExamYear(String uniMajorCode, String majorCode, String examYear) {
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findByUniMajorCodeAndMajorCodeAndExamYear(uniMajorCode, majorCode, examYear);
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoALatestVO> findByUniMajorCodeInAndExamYear(List<String> uniMajorCodes, String examYear){
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findByUniMajorCodeInAndExamYear(uniMajorCodes, examYear);
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
    ...
    @Override
    @Transactional
    public List<UniMajorInfoALatest> batchSave(List<UniMajorInfoALatest> list){
        for (int i = 0; i < list.size(); i++) {
    ...
    @Override
    @Transactional
    public List<UniMajorInfoALatest> batchUpdate(List<UniMajorInfoALatest> list){
        for (int i = 0; i < list.size(); i++) {
            if(i>0 && i % ConstDefine.BATCH_SIZE == 0){
                entityManager.flush(); // flush 触发事务 同步
                entityManager.clear(); // 清除缓存;  清除实体
...
//        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
    ...
    @Override
    public List<UniMajorInfoALatestVO> findByPiciAndExamYear(Integer pici, String examYear){
        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoALatest> inList = imp.findByPiciAndExamYear(pici, examYear);
        if (inList != null) {
            for (UniMajorInfoALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoALatestVO.class));
            }
