...
@Transactional
@Service
public class UniMajorInfoAServiceImpl  implements UniMajorInfoAService {


    @Autowired
    private UniMajorInfoARepo imp;
     ...
     */
    @Override
    public UniMajorInfoA saveUniMajorInfoA(UniMajorInfoAVO uniMajorInfoA) {

        return imp.save(BeanMapper.map(uniMajorInfoA, UniMajorInfoA.class));
    }
    /**
     * 删除单条记录
     * @param id
    ...
    public boolean deleteByKeyId(Long id) {
        UniMajorInfoA entity = imp.findById(id).orElse(null);
     ...
     */
    @Override
    public UniMajorInfoAVO findOneById(Long id) {
        UniMajorInfoA entity = imp.findById(id).orElse(null);
        if(entity == null) return null;
        return  BeanMapper.map(entity, UniMajorInfoAVO.class);
    }
    @Override
    public List<UniMajorInfoAVO> findByIdIn(List<Long> id) {
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findByIdIn(id);
        if(  inList != null){
            for (UniMajorInfoA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoAVO.class));
            }
        }
        return  voList;
    }    /**
     ...
     */
    @Override
    public List<UniMajorInfoAVO> getAllUniMajorInfoAs() {
        return BeanMapper.mapList(imp.findAll(), UniMajorInfoAVO.class);
    }
    /**
     * 更新记录
     ...
     */
    @Override
    public String updateUniMajorInfoA(UniMajorInfoAVO uniMajorInfoAVO) {
        UniMajorInfoA uniMajorInfoA = BeanMapper.map(uniMajorInfoAVO, UniMajorInfoA.class);
  ...
  public List<UniMajorInfoAVO> findByUniMajorCode(String uniMajorCode){
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findByUniMajorCode(uniMajorCode);
        if(  inList != null){
            for (UniMajorInfoA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoAVO.class));
            }
  ...
  public List<UniMajorInfoAVO> findByUniMajorCodeLike(String uniMajorCode){
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findByUniMajorCodeLike("%"+uniMajorCode+"%");
        if(  inList != null){
            for (UniMajorInfoA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoAVO.class));
            }
  ...
  public List<UniMajorInfoAVO> findByPici(Integer pici){
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findByPici(pici);
        if(  inList != null){
            for (UniMajorInfoA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoAVO.class));
            }
  ...
  public List<UniMajorInfoAVO> findBySchoolName(String schoolName){
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findBySchoolName(schoolName);
        if(  inList != null){
            for (UniMajorInfoA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoAVO.class));
            }
  ...
  public List<UniMajorInfoAVO> findBySchoolNameLike(String schoolName){
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findBySchoolNameLike("%"+schoolName+"%");
        if(  inList != null){
            for (UniMajorInfoA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoAVO.class));
            }
  ...
  public List<UniMajorInfoAVO> findBySchoolLvl1(Integer schoolLvl1){
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findBySchoolLvl1(schoolLvl1);
        if(  inList != null){
            for (UniMajorInfoA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoAVO.class));
            }
  ...
  public List<UniMajorInfoAVO> findBySchoolLvl2(Integer schoolLvl2){
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findBySchoolLvl2(schoolLvl2);
        if(  inList != null){
            for (UniMajorInfoA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoAVO.class));
            }
  ...
  public List<UniMajorInfoAVO> findBySchoolLvl3(Integer schoolLvl3){
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findBySchoolLvl3(schoolLvl3);
        if(  inList != null){
            for (UniMajorInfoA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoAVO.class));
            }
  ...
  public List<UniMajorInfoAVO> findBySchoolType(String schoolType){
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findBySchoolType(schoolType);
        if(  inList != null){
            for (UniMajorInfoA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoAVO.class));
            }
  ...
  public List<UniMajorInfoAVO> findBySchoolTypeLike(String schoolType){
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findBySchoolTypeLike("%"+schoolType+"%");
        if(  inList != null){
            for (UniMajorInfoA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoAVO.class));
            }
  ...
  public List<UniMajorInfoAVO> findByProvince(String province){
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findByProvince(province);
        if(  inList != null){
            for (UniMajorInfoA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoAVO.class));
            }
  ...
  public List<UniMajorInfoAVO> findByProvinceLike(String province){
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findByProvinceLike("%"+province+"%");
        if(  inList != null){
            for (UniMajorInfoA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoAVO.class));
            }
  ...
  public List<UniMajorInfoAVO> findByMajorName(String majorName){
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findByMajorName(majorName);
        if(  inList != null){
            for (UniMajorInfoA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoAVO.class));
            }
  ...
  public List<UniMajorInfoAVO> findByMajorNameLike(String majorName){
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findByMajorNameLike("%"+majorName+"%");
        if(  inList != null){
            for (UniMajorInfoA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoAVO.class));
            }
    ...
    public List<UniMajorInfoAVO> findByIsZhongWai(Integer isZhongWai){
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findByIsZhongWai(isZhongWai);
        if(  inList != null){
            for (UniMajorInfoA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoAVO.class));
            }
    ...
    public List<UniMajorInfoAVO> findByIsBenSuo(Integer isBenSuo){
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findByIsBenSuo(isBenSuo);
        if(  inList != null){
            for (UniMajorInfoA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoAVO.class));
            }
...
//    public Map<String, Object> listbyPage(String uniMajorCode,Integer pici,String schoolName,Integer schoolLvl1,Integer schoolLvl2,Integer schoolLvl3,String schoolType,String province,String majorName,Pageable pageable) {
        ...
        org.springframework.data.domain.Page<UniMajorInfoA> pageResult = null;
...
//        if (StringUtils.isEmpty(uniMajorCode)  && StringUtils.isEmpty(pici)  && StringUtils.isEmpty(schoolName)  && StringUtils.isEmpty(schoolLvl1)  && StringUtils.isEmpty(schoolLvl2)  && StringUtils.isEmpty(schoolLvl3)  && StringUtils.isEmpty(schoolType)  && StringUtils.isEmpty(province)  && StringUtils.isEmpty(majorName) ) {
            ...
            Specification<UniMajorInfoA> spec = (Specification<UniMajorInfoA>) (root, query, cb) -> {
...
//                        }
        ...
        List<UniMajorInfoAVO> list = BeanMapper.mapList(pageResult.getContent(), UniMajorInfoAVO.class);
    ...
    @Override
    public List<UniMajorInfoAVO> findByUniMajorCodeIn(List<String> ids)
    {
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findByUniMajorCodeIn( ids);
        if (inList != null) {
            for (UniMajorInfoA inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoAVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoAVO> findByUniMajorCodeAndExamYear(String uniMajorCode, String examYear) {
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findByUniMajorCodeAndExamYear(uniMajorCode, examYear);
        if (inList != null) {
            for (UniMajorInfoA inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoAVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoAVO> findByUniMajorCodeAndMajorCodeAndExamYear(String uniMajorCode, String majorCode, String examYear) {
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findByUniMajorCodeAndMajorCodeAndExamYear(uniMajorCode, majorCode, examYear);
        if (inList != null) {
            for (UniMajorInfoA inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoAVO.class));
            }
    ...
    @Override
    @Transactional
    public List<UniMajorInfoA> batchSave(List<UniMajorInfoA> entityList){
//        List<UniMajorInfoA> entityList = new ArrayList<>();
//        for (int i = 0; i < list.size(); i++) {
//            entityList.add(BeanMapper.map(list.get(i), UniMajorInfoA.class));
//        }
...
//            entityManager.persist(BeanMapper.map(list.get(i), UniMajorInfoA.class));
    ...
    @Override
    @Transactional
    public List<UniMajorInfoA> batchUpdate(List<UniMajorInfoA> list){
        for (int i = 0; i < list.size(); i++) {
            if(i>0 && i % ConstDefine.BATCH_SIZE == 0){
                entityManager.flush(); // flush 触发事务 同步
                entityManager.clear(); // 清除缓存;  清除实体
...
//        List<UniMajorInfoALatestVO> voList = new ArrayList<>();
    ...
    @Override
    public List<UniMajorInfoAVO> findByPiciAndExamYear(Integer pici, String examYear){
        List<UniMajorInfoAVO> voList = new ArrayList<>();
        List<UniMajorInfoA> inList = imp.findByPiciAndExamYear(pici, examYear);
        if (inList != null) {
            for (UniMajorInfoA inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoAVO.class));
            }
