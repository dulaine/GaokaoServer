...
@Service
public class UniMajorInfoPreALatestServiceImpl implements UniMajorInfoPreALatestService {


    @Autowired
    private UniMajorInfoPreALatestRepo imp;
     ...
     */
    @Override
    public UniMajorInfoPreALatest saveUniMajorInfoPreALatest(UniMajorInfoPreALatestVO uniMajorInfoALatest) {

        return imp.save(BeanMapper.map(uniMajorInfoALatest, UniMajorInfoPreALatest.class));
    }
    ...
    public boolean deleteByKeyId(Long id) {
        UniMajorInfoPreALatest entity = imp.findById(id).orElse(null);
     ...
     */
    @Override
    public UniMajorInfoPreALatestVO findOneById(Long id) {
        UniMajorInfoPreALatest entity = imp.findById(id).orElse(null);
        if (entity == null) return null;
        return BeanMapper.map(entity, UniMajorInfoPreALatestVO.class);
    }

    @Override
    public List<UniMajorInfoPreALatestVO> findByIdIn(List<Long> id) {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findByIdIn(id);
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
        }
        return voList;
    }
     ...
     */
    @Override
    public List<UniMajorInfoPreALatestVO> getAllUniMajorInfoPreALatests() {
        return BeanMapper.mapList(imp.findAll(), UniMajorInfoPreALatestVO.class);
    }
     ...
     */
    @Override
    public String updateUniMajorInfoPreALatest(UniMajorInfoPreALatestVO uniMajorInfoALatestVO) {
        UniMajorInfoPreALatest uniMajorInfoALatest = BeanMapper.map(uniMajorInfoALatestVO, UniMajorInfoPreALatest.class);
     ...
     */
    @Override
    public List<UniMajorInfoPreALatestVO> findByUniMajorCode(String uniMajorCode) {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findByUniMajorCode(uniMajorCode);
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
     ...
     */
    @Override
    public List<UniMajorInfoPreALatestVO> findByUniMajorCodeLike(String uniMajorCode) {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findByUniMajorCodeLike("%" + uniMajorCode + "%");
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
     ...
     */
    @Override
    public List<UniMajorInfoPreALatestVO> findByPici(Integer pici) {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findByPici(pici);
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
     ...
     */
    @Override
    public List<UniMajorInfoPreALatestVO> findBySchoolName(String schoolName) {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findBySchoolName(schoolName);
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
     ...
     */
    @Override
    public List<UniMajorInfoPreALatestVO> findBySchoolNameLike(String schoolName) {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findBySchoolNameLike("%" + schoolName + "%");
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
     ...
     */
    @Override
    public List<UniMajorInfoPreALatestVO> findBySchoolLvl1(Integer schoolLvl1) {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findBySchoolLvl1(schoolLvl1);
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
     ...
     */
    @Override
    public List<UniMajorInfoPreALatestVO> findBySchoolLvl2(Integer schoolLvl2) {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findBySchoolLvl2(schoolLvl2);
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
     ...
     */
    @Override
    public List<UniMajorInfoPreALatestVO> findBySchoolLvl3(Integer schoolLvl3) {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findBySchoolLvl3(schoolLvl3);
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
     ...
     */
    @Override
    public List<UniMajorInfoPreALatestVO> findBySchoolType(String schoolType) {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findBySchoolType(schoolType);
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
     ...
     */
    @Override
    public List<UniMajorInfoPreALatestVO> findBySchoolTypeLike(String schoolType) {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findBySchoolTypeLike("%" + schoolType + "%");
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
     ...
     */
    @Override
    public List<UniMajorInfoPreALatestVO> findByProvince(String province) {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findByProvince(province);
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
     ...
     */
    @Override
    public List<UniMajorInfoPreALatestVO> findByProvinceLike(String province) {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findByProvinceLike("%" + province + "%");
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
     ...
     */
    @Override
    public List<UniMajorInfoPreALatestVO> findByMajorName(String majorName) {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findByMajorName(majorName);
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
     ...
     */
    @Override
    public List<UniMajorInfoPreALatestVO> findByMajorNameLike(String majorName) {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findByMajorNameLike("%" + majorName + "%");
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
     ...
     */
    @Override
    public List<UniMajorInfoPreALatestVO> findByIsZhongWai(Integer isZhongWai){
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findByIsZhongWai(isZhongWai);
        if(  inList != null){
            for (UniMajorInfoPreALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreALatestVO.class));
            }
     ...
     */
    @Override
    public List<UniMajorInfoPreALatestVO> findByIsBenSuo(Integer isBenSuo){
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findByIsBenSuo(isBenSuo);
        if(  inList != null){
            for (UniMajorInfoPreALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreALatestVO.class));
            }
...
//    public Map<String, Object> listbyPage(String uniMajorCode,Integer pici,String schoolName,Integer schoolLvl1,Integer schoolLvl2,Integer schoolLvl3,String schoolType,String province,String majorName,Pageable pageable) {
        ...
        org.springframework.data.domain.Page<UniMajorInfoPreALatest> pageResult = null;
...
//            boolean finalIsMajor2Code = isMajor2Code;
            ...
            Specification<UniMajorInfoPreALatest> spec = (Specification<UniMajorInfoPreALatest>) (root, query, cb) -> {
...
//            pageResult = imp.findPage(spec, pageable);
        ...
        List<UniMajorInfoPreALatestVO> list = BeanMapper.mapList(pageResult.getContent(), UniMajorInfoPreALatestVO.class);
...
//        }
    ...
    @Override
    public List<UniMajorInfoPreALatestVO> findByUniMajorCodeIn(List<String> ids) {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findByUniMajorCodeIn(ids);
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoPreALatestVO> findByUniMajorCodeAndExamYear(String uniMajorCode, String examYear) {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findByUniMajorCodeAndExamYear(uniMajorCode, examYear);
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoPreALatestVO> findByHistMajorIdsAndExamYear(List<Long> histDBIds, String examYear)
    {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findByHistMajorIdsAndExamYear( histDBIds, examYear);
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoPreALatestVO> findByUniMajorCodeAndMajorCodeAndExamYear(String uniMajorCode, String majorCode, String examYear) {
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findByUniMajorCodeAndMajorCodeAndExamYear(uniMajorCode, majorCode, examYear);
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoPreALatestVO> findByUniMajorCodeInAndExamYear(List<String> uniMajorCodes, String examYear){
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findByUniMajorCodeInAndExamYear(uniMajorCodes, examYear);
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
    ...
    @Override
    @Transactional
    public List<UniMajorInfoPreALatest> batchSave(List<UniMajorInfoPreALatest> list){
        for (int i = 0; i < list.size(); i++) {
    ...
    @Override
    @Transactional
    public List<UniMajorInfoPreALatest> batchUpdate(List<UniMajorInfoPreALatest> list){
        for (int i = 0; i < list.size(); i++) {
            if(i>0 && i % ConstDefine.BATCH_SIZE == 0){
                entityManager.flush(); // flush 触发事务 同步
                entityManager.clear(); // 清除缓存;  清除实体
...
//        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
    ...
    @Override
    public List<UniMajorInfoPreALatestVO> findByPiciAndExamYear(Integer pici, String examYear){
        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoPreALatest> inList = imp.findByPiciAndExamYear(pici, examYear);
        if (inList != null) {
            for (UniMajorInfoPreALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreALatestVO.class));
            }
