...
@Transactional
@Service
public class UniMajorInfoPreAServiceImpl  implements UniMajorInfoPreAService {


    @Autowired
    private UniMajorInfoPreARepo imp;
     ...
     */
    @Override
    public UniMajorInfoPreA saveUniMajorInfoPreA(UniMajorInfoPreAVO uniMajorInfoA) {

        return imp.save(BeanMapper.map(uniMajorInfoA, UniMajorInfoPreA.class));
    }
    /**
     * 删除单条记录
     * @param id
    ...
    public boolean deleteByKeyId(Long id) {
        UniMajorInfoPreA entity = imp.findById(id).orElse(null);
     ...
     */
    @Override
    public UniMajorInfoPreAVO findOneById(Long id) {
        UniMajorInfoPreA entity = imp.findById(id).orElse(null);
        if(entity == null) return null;
        return  BeanMapper.map(entity, UniMajorInfoPreAVO.class);
    }
    @Override
    public List<UniMajorInfoPreAVO> findByIdIn(List<Long> id) {
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findByIdIn(id);
        if(  inList != null){
            for (UniMajorInfoPreA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreAVO.class));
            }
        }
        return  voList;
    }    /**
     ...
     */
    @Override
    public List<UniMajorInfoPreAVO> getAllUniMajorInfoPreAs() {
        return BeanMapper.mapList(imp.findAll(), UniMajorInfoPreAVO.class);
    }
    /**
     * 更新记录
     ...
     */
    @Override
    public String updateUniMajorInfoPreA(UniMajorInfoPreAVO uniMajorInfoAVO) {
        UniMajorInfoPreA uniMajorInfoA = BeanMapper.map(uniMajorInfoAVO, UniMajorInfoPreA.class);
  ...
  public List<UniMajorInfoPreAVO> findByUniMajorCode(String uniMajorCode){
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findByUniMajorCode(uniMajorCode);
        if(  inList != null){
            for (UniMajorInfoPreA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreAVO.class));
            }
  ...
  public List<UniMajorInfoPreAVO> findByUniMajorCodeLike(String uniMajorCode){
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findByUniMajorCodeLike("%"+uniMajorCode+"%");
        if(  inList != null){
            for (UniMajorInfoPreA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreAVO.class));
            }
  ...
  public List<UniMajorInfoPreAVO> findByPici(Integer pici){
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findByPici(pici);
        if(  inList != null){
            for (UniMajorInfoPreA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreAVO.class));
            }
  ...
  public List<UniMajorInfoPreAVO> findBySchoolName(String schoolName){
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findBySchoolName(schoolName);
        if(  inList != null){
            for (UniMajorInfoPreA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreAVO.class));
            }
  ...
  public List<UniMajorInfoPreAVO> findBySchoolNameLike(String schoolName){
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findBySchoolNameLike("%"+schoolName+"%");
        if(  inList != null){
            for (UniMajorInfoPreA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreAVO.class));
            }
  ...
  public List<UniMajorInfoPreAVO> findBySchoolLvl1(Integer schoolLvl1){
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findBySchoolLvl1(schoolLvl1);
        if(  inList != null){
            for (UniMajorInfoPreA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreAVO.class));
            }
  ...
  public List<UniMajorInfoPreAVO> findBySchoolLvl2(Integer schoolLvl2){
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findBySchoolLvl2(schoolLvl2);
        if(  inList != null){
            for (UniMajorInfoPreA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreAVO.class));
            }
  ...
  public List<UniMajorInfoPreAVO> findBySchoolLvl3(Integer schoolLvl3){
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findBySchoolLvl3(schoolLvl3);
        if(  inList != null){
            for (UniMajorInfoPreA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreAVO.class));
            }
  ...
  public List<UniMajorInfoPreAVO> findBySchoolType(String schoolType){
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findBySchoolType(schoolType);
        if(  inList != null){
            for (UniMajorInfoPreA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreAVO.class));
            }
  ...
  public List<UniMajorInfoPreAVO> findBySchoolTypeLike(String schoolType){
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findBySchoolTypeLike("%"+schoolType+"%");
        if(  inList != null){
            for (UniMajorInfoPreA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreAVO.class));
            }
  ...
  public List<UniMajorInfoPreAVO> findByProvince(String province){
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findByProvince(province);
        if(  inList != null){
            for (UniMajorInfoPreA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreAVO.class));
            }
  ...
  public List<UniMajorInfoPreAVO> findByProvinceLike(String province){
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findByProvinceLike("%"+province+"%");
        if(  inList != null){
            for (UniMajorInfoPreA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreAVO.class));
            }
  ...
  public List<UniMajorInfoPreAVO> findByMajorName(String majorName){
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findByMajorName(majorName);
        if(  inList != null){
            for (UniMajorInfoPreA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreAVO.class));
            }
  ...
  public List<UniMajorInfoPreAVO> findByMajorNameLike(String majorName){
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findByMajorNameLike("%"+majorName+"%");
        if(  inList != null){
            for (UniMajorInfoPreA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreAVO.class));
            }
    ...
    public List<UniMajorInfoPreAVO> findByIsZhongWai(Integer isZhongWai){
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findByIsZhongWai(isZhongWai);
        if(  inList != null){
            for (UniMajorInfoPreA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreAVO.class));
            }
    ...
    public List<UniMajorInfoPreAVO> findByIsBenSuo(Integer isBenSuo){
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findByIsBenSuo(isBenSuo);
        if(  inList != null){
            for (UniMajorInfoPreA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreAVO.class));
            }
...
//    public Map<String, Object> listbyPage(String uniMajorCode,Integer pici,String schoolName,Integer schoolLvl1,Integer schoolLvl2,Integer schoolLvl3,String schoolType,String province,String majorName,Pageable pageable) {
        ...
        org.springframework.data.domain.Page<UniMajorInfoPreA> pageResult = null;
...
//        if (StringUtils.isEmpty(uniMajorCode)  && StringUtils.isEmpty(pici)  && StringUtils.isEmpty(schoolName)  && StringUtils.isEmpty(schoolLvl1)  && StringUtils.isEmpty(schoolLvl2)  && StringUtils.isEmpty(schoolLvl3)  && StringUtils.isEmpty(schoolType)  && StringUtils.isEmpty(province)  && StringUtils.isEmpty(majorName) ) {
            ...
            Specification<UniMajorInfoPreA> spec = (Specification<UniMajorInfoPreA>) (root, query, cb) -> {
...
//                        }
        ...
        List<UniMajorInfoPreAVO> list = BeanMapper.mapList(pageResult.getContent(), UniMajorInfoPreAVO.class);
    ...
    @Override
    public List<UniMajorInfoPreAVO> findByUniMajorCodeIn(List<String> ids)
    {
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findByUniMajorCodeIn( ids);
        if (inList != null) {
            for (UniMajorInfoPreA inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreAVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoPreAVO> findByUniMajorCodeAndExamYear(String uniMajorCode, String examYear) {
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findByUniMajorCodeAndExamYear(uniMajorCode, examYear);
        if (inList != null) {
            for (UniMajorInfoPreA inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreAVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoPreAVO> findByUniMajorCodeAndMajorCodeAndExamYear(String uniMajorCode, String majorCode, String examYear) {
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findByUniMajorCodeAndMajorCodeAndExamYear(uniMajorCode, majorCode, examYear);
        if (inList != null) {
            for (UniMajorInfoPreA inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreAVO.class));
            }
    ...
    @Override
    @Transactional
    public List<UniMajorInfoPreA> batchSave(List<UniMajorInfoPreA> entityList){
//        List<UniMajorInfoPreA> entityList = new ArrayList<>();
//        for (int i = 0; i < list.size(); i++) {
//            entityList.add(BeanMapper.map(list.get(i), UniMajorInfoPreA.class));
//        }
...
//            entityManager.persist(BeanMapper.map(list.get(i), UniMajorInfoPreA.class));
    ...
    @Override
    @Transactional
    public List<UniMajorInfoPreA> batchUpdate(List<UniMajorInfoPreA> list){
        for (int i = 0; i < list.size(); i++) {
            if(i>0 && i % ConstDefine.BATCH_SIZE == 0){
                entityManager.flush(); // flush 触发事务 同步
                entityManager.clear(); // 清除缓存;  清除实体
...
//        List<UniMajorInfoPreALatestVO> voList = new ArrayList<>();
    ...
    @Override
    public List<UniMajorInfoPreAVO> findByPiciAndExamYear(Integer pici, String examYear){
        List<UniMajorInfoPreAVO> voList = new ArrayList<>();
        List<UniMajorInfoPreA> inList = imp.findByPiciAndExamYear(pici, examYear);
        if (inList != null) {
            for (UniMajorInfoPreA inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreAVO.class));
            }
