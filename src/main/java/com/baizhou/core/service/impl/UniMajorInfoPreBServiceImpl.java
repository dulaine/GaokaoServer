...
@Transactional
@Service
public class UniMajorInfoPreBServiceImpl  implements UniMajorInfoPreBService {


    @Autowired
    private UniMajorInfoPreBRepo imp;
     ...
     */
    @Override
    public UniMajorInfoPreB saveUniMajorInfoPreB(UniMajorInfoPreBVO uniMajorInfoA) {

        return imp.save(BeanMapper.map(uniMajorInfoA, UniMajorInfoPreB.class));
    }
    /**
     * 删除单条记录
     * @param id
    ...
    public boolean deleteByKeyId(Long id) {
        UniMajorInfoPreB entity = imp.findById(id).orElse(null);
     ...
     */
    @Override
    public UniMajorInfoPreBVO findOneById(Long id) {
        UniMajorInfoPreB entity = imp.findById(id).orElse(null);
        if(entity == null) return null;
        return  BeanMapper.map(entity, UniMajorInfoPreBVO.class);
    }
    @Override
    public List<UniMajorInfoPreBVO> findByIdIn(List<Long> id) {
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findByIdIn(id);
        if(  inList != null){
            for (UniMajorInfoPreB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreBVO.class));
            }
        }
        return  voList;
    }    /**
     ...
     */
    @Override
    public List<UniMajorInfoPreBVO> getAllUniMajorInfoPreBs() {
        return BeanMapper.mapList(imp.findAll(), UniMajorInfoPreBVO.class);
    }
    /**
     * 更新记录
     ...
     */
    @Override
    public String updateUniMajorInfoPreB(UniMajorInfoPreBVO uniMajorInfoAVO) {
        UniMajorInfoPreB uniMajorInfoA = BeanMapper.map(uniMajorInfoAVO, UniMajorInfoPreB.class);
  ...
  public List<UniMajorInfoPreBVO> findByUniMajorCode(String uniMajorCode){
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findByUniMajorCode(uniMajorCode);
        if(  inList != null){
            for (UniMajorInfoPreB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreBVO.class));
            }
  ...
  public List<UniMajorInfoPreBVO> findByUniMajorCodeLike(String uniMajorCode){
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findByUniMajorCodeLike("%"+uniMajorCode+"%");
        if(  inList != null){
            for (UniMajorInfoPreB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreBVO.class));
            }
  ...
  public List<UniMajorInfoPreBVO> findByPici(Integer pici){
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findByPici(pici);
        if(  inList != null){
            for (UniMajorInfoPreB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreBVO.class));
            }
  ...
  public List<UniMajorInfoPreBVO> findBySchoolName(String schoolName){
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findBySchoolName(schoolName);
        if(  inList != null){
            for (UniMajorInfoPreB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreBVO.class));
            }
  ...
  public List<UniMajorInfoPreBVO> findBySchoolNameLike(String schoolName){
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findBySchoolNameLike("%"+schoolName+"%");
        if(  inList != null){
            for (UniMajorInfoPreB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreBVO.class));
            }
  ...
  public List<UniMajorInfoPreBVO> findBySchoolLvl1(Integer schoolLvl1){
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findBySchoolLvl1(schoolLvl1);
        if(  inList != null){
            for (UniMajorInfoPreB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreBVO.class));
            }
  ...
  public List<UniMajorInfoPreBVO> findBySchoolLvl2(Integer schoolLvl2){
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findBySchoolLvl2(schoolLvl2);
        if(  inList != null){
            for (UniMajorInfoPreB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreBVO.class));
            }
  ...
  public List<UniMajorInfoPreBVO> findBySchoolLvl3(Integer schoolLvl3){
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findBySchoolLvl3(schoolLvl3);
        if(  inList != null){
            for (UniMajorInfoPreB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreBVO.class));
            }
  ...
  public List<UniMajorInfoPreBVO> findBySchoolType(String schoolType){
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findBySchoolType(schoolType);
        if(  inList != null){
            for (UniMajorInfoPreB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreBVO.class));
            }
  ...
  public List<UniMajorInfoPreBVO> findBySchoolTypeLike(String schoolType){
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findBySchoolTypeLike("%"+schoolType+"%");
        if(  inList != null){
            for (UniMajorInfoPreB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreBVO.class));
            }
  ...
  public List<UniMajorInfoPreBVO> findByProvince(String province){
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findByProvince(province);
        if(  inList != null){
            for (UniMajorInfoPreB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreBVO.class));
            }
  ...
  public List<UniMajorInfoPreBVO> findByProvinceLike(String province){
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findByProvinceLike("%"+province+"%");
        if(  inList != null){
            for (UniMajorInfoPreB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreBVO.class));
            }
  ...
  public List<UniMajorInfoPreBVO> findByMajorName(String majorName){
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findByMajorName(majorName);
        if(  inList != null){
            for (UniMajorInfoPreB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreBVO.class));
            }
  ...
  public List<UniMajorInfoPreBVO> findByMajorNameLike(String majorName){
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findByMajorNameLike("%"+majorName+"%");
        if(  inList != null){
            for (UniMajorInfoPreB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreBVO.class));
            }
    ...
    public List<UniMajorInfoPreBVO> findByIsZhongWai(Integer isZhongWai){
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findByIsZhongWai(isZhongWai);
        if(  inList != null){
            for (UniMajorInfoPreB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreBVO.class));
            }
    ...
    public List<UniMajorInfoPreBVO> findByIsBenSuo(Integer isBenSuo){
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findByIsBenSuo(isBenSuo);
        if(  inList != null){
            for (UniMajorInfoPreB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoPreBVO.class));
            }
...
//    public Map<String, Object> listbyPage(String uniMajorCode,Integer pici,String schoolName,Integer schoolLvl1,Integer schoolLvl2,Integer schoolLvl3,String schoolType,String province,String majorName,Pageable pageable) {
        ...
        org.springframework.data.domain.Page<UniMajorInfoPreB> pageResult = null;
...
//        if (StringUtils.isEmpty(uniMajorCode)  && StringUtils.isEmpty(pici)  && StringUtils.isEmpty(schoolName)  && StringUtils.isEmpty(schoolLvl1)  && StringUtils.isEmpty(schoolLvl2)  && StringUtils.isEmpty(schoolLvl3)  && StringUtils.isEmpty(schoolType)  && StringUtils.isEmpty(province)  && StringUtils.isEmpty(majorName) ) {
            ...
            Specification<UniMajorInfoPreB> spec = (Specification<UniMajorInfoPreB>) (root, query, cb) -> {
...
//                        }
        ...
        List<UniMajorInfoPreBVO> list = BeanMapper.mapList(pageResult.getContent(), UniMajorInfoPreBVO.class);
    ...
    @Override
    public List<UniMajorInfoPreBVO> findByUniMajorCodeIn(List<String> ids)
    {
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findByUniMajorCodeIn( ids);
        if (inList != null) {
            for (UniMajorInfoPreB inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreBVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoPreBVO> findByUniMajorCodeAndExamYear(String uniMajorCode, String examYear) {
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findByUniMajorCodeAndExamYear(uniMajorCode, examYear);
        if (inList != null) {
            for (UniMajorInfoPreB inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreBVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoPreBVO> findByUniMajorCodeAndMajorCodeAndExamYear(String uniMajorCode, String majorCode, String examYear) {
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findByUniMajorCodeAndMajorCodeAndExamYear(uniMajorCode, majorCode, examYear);
        if (inList != null) {
            for (UniMajorInfoPreB inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreBVO.class));
            }
    ...
    @Override
    @Transactional
    public List<UniMajorInfoPreB> batchSave(List<UniMajorInfoPreB> entityList){
//        List<UniMajorInfoPreB> entityList = new ArrayList<>();
//        for (int i = 0; i < list.size(); i++) {
//            entityList.add(BeanMapper.map(list.get(i), UniMajorInfoPreB.class));
//        }
...
//            entityManager.persist(BeanMapper.map(list.get(i), UniMajorInfoPreB.class));
    ...
    @Override
    @Transactional
    public List<UniMajorInfoPreB> batchUpdate(List<UniMajorInfoPreB> list){
        for (int i = 0; i < list.size(); i++) {
            if(i>0 && i % ConstDefine.BATCH_SIZE == 0){
                entityManager.flush(); // flush 触发事务 同步
                entityManager.clear(); // 清除缓存;  清除实体
...
//        List<UniMajorInfoPreBLatestVO> voList = new ArrayList<>();
    ...
    @Override
    public List<UniMajorInfoPreBVO> findByPiciAndExamYear(Integer pici, String examYear){
        List<UniMajorInfoPreBVO> voList = new ArrayList<>();
        List<UniMajorInfoPreB> inList = imp.findByPiciAndExamYear(pici, examYear);
        if (inList != null) {
            for (UniMajorInfoPreB inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoPreBVO.class));
            }
