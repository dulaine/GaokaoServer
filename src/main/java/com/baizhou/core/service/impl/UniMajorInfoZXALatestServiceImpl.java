...
@Service
public class UniMajorInfoZXALatestServiceImpl implements UniMajorInfoZXALatestService {


    @Autowired
    private UniMajorInfoZXALatestRepo imp;
     ...
     */
    @Override
    public UniMajorInfoZXALatest saveUniMajorInfoZXALatest(UniMajorInfoZXALatestVO uniMajorInfoALatest) {

        return imp.save(BeanMapper.map(uniMajorInfoALatest, UniMajorInfoZXALatest.class));
    }
    ...
    public boolean deleteByKeyId(Long id) {
        UniMajorInfoZXALatest entity = imp.findById(id).orElse(null);
     ...
     */
    @Override
    public UniMajorInfoZXALatestVO findOneById(Long id) {
        UniMajorInfoZXALatest entity = imp.findById(id).orElse(null);
        if (entity == null) return null;
        return BeanMapper.map(entity, UniMajorInfoZXALatestVO.class);
    }

    @Override
    public List<UniMajorInfoZXALatestVO> findByIdIn(List<Long> id) {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findByIdIn(id);
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
        }
        return voList;
    }
     ...
     */
    @Override
    public List<UniMajorInfoZXALatestVO> getAllUniMajorInfoZXALatests() {
        return BeanMapper.mapList(imp.findAll(), UniMajorInfoZXALatestVO.class);
    }
     ...
     */
    @Override
    public String updateUniMajorInfoZXALatest(UniMajorInfoZXALatestVO uniMajorInfoALatestVO) {
        UniMajorInfoZXALatest uniMajorInfoALatest = BeanMapper.map(uniMajorInfoALatestVO, UniMajorInfoZXALatest.class);
    ...
    public List<UniMajorInfoZXALatestVO> findByUniMajorCode(String uniMajorCode) {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findByUniMajorCode(uniMajorCode);
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    public List<UniMajorInfoZXALatestVO> findByUniMajorCodeLike(String uniMajorCode) {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findByUniMajorCodeLike("%" + uniMajorCode + "%");
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    public List<UniMajorInfoZXALatestVO> findByPici(Integer pici) {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findByPici(pici);
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    public List<UniMajorInfoZXALatestVO> findBySchoolName(String schoolName) {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findBySchoolName(schoolName);
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    public List<UniMajorInfoZXALatestVO> findBySchoolNameLike(String schoolName) {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findBySchoolNameLike("%" + schoolName + "%");
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    public List<UniMajorInfoZXALatestVO> findBySchoolLvl1(Integer schoolLvl1) {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findBySchoolLvl1(schoolLvl1);
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    public List<UniMajorInfoZXALatestVO> findBySchoolLvl2(Integer schoolLvl2) {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findBySchoolLvl2(schoolLvl2);
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    public List<UniMajorInfoZXALatestVO> findBySchoolLvl3(Integer schoolLvl3) {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findBySchoolLvl3(schoolLvl3);
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    public List<UniMajorInfoZXALatestVO> findBySchoolType(String schoolType) {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findBySchoolType(schoolType);
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    public List<UniMajorInfoZXALatestVO> findBySchoolTypeLike(String schoolType) {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findBySchoolTypeLike("%" + schoolType + "%");
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    public List<UniMajorInfoZXALatestVO> findByProvince(String province) {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findByProvince(province);
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    public List<UniMajorInfoZXALatestVO> findByProvinceLike(String province) {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findByProvinceLike("%" + province + "%");
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    public List<UniMajorInfoZXALatestVO> findByMajorName(String majorName) {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findByMajorName(majorName);
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    public List<UniMajorInfoZXALatestVO> findByMajorNameLike(String majorName) {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findByMajorNameLike("%" + majorName + "%");
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    public List<UniMajorInfoZXALatestVO> findByIsZhongWai(Integer isZhongWai){
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findByIsZhongWai(isZhongWai);
        if(  inList != null){
            for (UniMajorInfoZXALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXALatestVO.class));
            }
    ...
    public List<UniMajorInfoZXALatestVO> findByIsBenSuo(Integer isBenSuo){
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findByIsBenSuo(isBenSuo);
        if(  inList != null){
            for (UniMajorInfoZXALatest inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXALatestVO.class));
            }
...
//    public Map<String, Object> listbyPage(String uniMajorCode,Integer pici,String schoolName,Integer schoolLvl1,Integer schoolLvl2,Integer schoolLvl3,String schoolType,String province,String majorName,Pageable pageable) {
        ...
        org.springframework.data.domain.Page<UniMajorInfoZXALatest> pageResult = null;
...
//            boolean finalIsMajor2Code = isMajor2Code;
            ...
            Specification<UniMajorInfoZXALatest> spec = (Specification<UniMajorInfoZXALatest>) (root, query, cb) -> {
...
//            pageResult = imp.findPage(spec, pageable);
        ...
        List<UniMajorInfoZXALatestVO> list = BeanMapper.mapList(pageResult.getContent(), UniMajorInfoZXALatestVO.class);
...
//        }
    ...
    @Override
    public List<UniMajorInfoZXALatestVO> findByUniMajorCodeIn(List<String> ids) {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findByUniMajorCodeIn(ids);
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoZXALatestVO> findByUniMajorCodeAndExamYear(String uniMajorCode, String examYear) {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findByUniMajorCodeAndExamYear(uniMajorCode, examYear);
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoZXALatestVO> findByHistMajorIdsAndExamYear(List<Long> histDBIds, String examYear)
    {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findByHistMajorIdsAndExamYear( histDBIds, examYear);
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoZXALatestVO> findByUniMajorCodeAndMajorCodeAndExamYear(String uniMajorCode, String majorCode, String examYear) {
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findByUniMajorCodeAndMajorCodeAndExamYear(uniMajorCode, majorCode, examYear);
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoZXALatestVO> findByUniMajorCodeInAndExamYear(List<String> uniMajorCodes, String examYear){
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findByUniMajorCodeInAndExamYear(uniMajorCodes, examYear);
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
    ...
    @Override
    @Transactional
    public List<UniMajorInfoZXALatest> batchSave(List<UniMajorInfoZXALatest> list){
        for (int i = 0; i < list.size(); i++) {
    ...
    @Override
    @Transactional
    public List<UniMajorInfoZXALatest> batchUpdate(List<UniMajorInfoZXALatest> list){
        for (int i = 0; i < list.size(); i++) {
            if(i>0 && i % ConstDefine.BATCH_SIZE == 0){
                entityManager.flush(); // flush 触发事务 同步
                entityManager.clear(); // 清除缓存;  清除实体
...
//        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
    ...
    @Override
    public List<UniMajorInfoZXALatestVO> findByPiciAndExamYear(Integer pici, String examYear){
        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
        List<UniMajorInfoZXALatest> inList = imp.findByPiciAndExamYear(pici, examYear);
        if (inList != null) {
            for (UniMajorInfoZXALatest inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXALatestVO.class));
            }
