...
@Transactional
@Service
public class UniMajorInfoZXAServiceImpl  implements UniMajorInfoZXAService {


    @Autowired
    private UniMajorInfoZXARepo imp;
     ...
     */
    @Override
    public UniMajorInfoZXA saveUniMajorInfoZXA(UniMajorInfoZXAVO uniMajorInfoA) {

        return imp.save(BeanMapper.map(uniMajorInfoA, UniMajorInfoZXA.class));
    }
    /**
     * 删除单条记录
     * @param id
    ...
    public boolean deleteByKeyId(Long id) {
        UniMajorInfoZXA entity = imp.findById(id).orElse(null);
     ...
     */
    @Override
    public UniMajorInfoZXAVO findOneById(Long id) {
        UniMajorInfoZXA entity = imp.findById(id).orElse(null);
        if(entity == null) return null;
        return  BeanMapper.map(entity, UniMajorInfoZXAVO.class);
    }
    @Override
    public List<UniMajorInfoZXAVO> findByIdIn(List<Long> id) {
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findByIdIn(id);
        if(  inList != null){
            for (UniMajorInfoZXA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXAVO.class));
            }
        }
        return  voList;
    }    /**
     ...
     */
    @Override
    public List<UniMajorInfoZXAVO> getAllUniMajorInfoZXAs() {
        return BeanMapper.mapList(imp.findAll(), UniMajorInfoZXAVO.class);
    }
    /**
     * 更新记录
     ...
     */
    @Override
    public String updateUniMajorInfoZXA(UniMajorInfoZXAVO uniMajorInfoAVO) {
        UniMajorInfoZXA uniMajorInfoA = BeanMapper.map(uniMajorInfoAVO, UniMajorInfoZXA.class);
  ...
  public List<UniMajorInfoZXAVO> findByUniMajorCode(String uniMajorCode){
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findByUniMajorCode(uniMajorCode);
        if(  inList != null){
            for (UniMajorInfoZXA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXAVO.class));
            }
  ...
  public List<UniMajorInfoZXAVO> findByUniMajorCodeLike(String uniMajorCode){
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findByUniMajorCodeLike("%"+uniMajorCode+"%");
        if(  inList != null){
            for (UniMajorInfoZXA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXAVO.class));
            }
  ...
  public List<UniMajorInfoZXAVO> findByPici(Integer pici){
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findByPici(pici);
        if(  inList != null){
            for (UniMajorInfoZXA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXAVO.class));
            }
  ...
  public List<UniMajorInfoZXAVO> findBySchoolName(String schoolName){
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findBySchoolName(schoolName);
        if(  inList != null){
            for (UniMajorInfoZXA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXAVO.class));
            }
  ...
  public List<UniMajorInfoZXAVO> findBySchoolNameLike(String schoolName){
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findBySchoolNameLike("%"+schoolName+"%");
        if(  inList != null){
            for (UniMajorInfoZXA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXAVO.class));
            }
  ...
  public List<UniMajorInfoZXAVO> findBySchoolLvl1(Integer schoolLvl1){
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findBySchoolLvl1(schoolLvl1);
        if(  inList != null){
            for (UniMajorInfoZXA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXAVO.class));
            }
  ...
  public List<UniMajorInfoZXAVO> findBySchoolLvl2(Integer schoolLvl2){
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findBySchoolLvl2(schoolLvl2);
        if(  inList != null){
            for (UniMajorInfoZXA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXAVO.class));
            }
  ...
  public List<UniMajorInfoZXAVO> findBySchoolLvl3(Integer schoolLvl3){
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findBySchoolLvl3(schoolLvl3);
        if(  inList != null){
            for (UniMajorInfoZXA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXAVO.class));
            }
  ...
  public List<UniMajorInfoZXAVO> findBySchoolType(String schoolType){
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findBySchoolType(schoolType);
        if(  inList != null){
            for (UniMajorInfoZXA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXAVO.class));
            }
  ...
  public List<UniMajorInfoZXAVO> findBySchoolTypeLike(String schoolType){
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findBySchoolTypeLike("%"+schoolType+"%");
        if(  inList != null){
            for (UniMajorInfoZXA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXAVO.class));
            }
  ...
  public List<UniMajorInfoZXAVO> findByProvince(String province){
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findByProvince(province);
        if(  inList != null){
            for (UniMajorInfoZXA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXAVO.class));
            }
  ...
  public List<UniMajorInfoZXAVO> findByProvinceLike(String province){
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findByProvinceLike("%"+province+"%");
        if(  inList != null){
            for (UniMajorInfoZXA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXAVO.class));
            }
  ...
  public List<UniMajorInfoZXAVO> findByMajorName(String majorName){
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findByMajorName(majorName);
        if(  inList != null){
            for (UniMajorInfoZXA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXAVO.class));
            }
  ...
  public List<UniMajorInfoZXAVO> findByMajorNameLike(String majorName){
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findByMajorNameLike("%"+majorName+"%");
        if(  inList != null){
            for (UniMajorInfoZXA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXAVO.class));
            }
    ...
    public List<UniMajorInfoZXAVO> findByIsZhongWai(Integer isZhongWai){
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findByIsZhongWai(isZhongWai);
        if(  inList != null){
            for (UniMajorInfoZXA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXAVO.class));
            }
    ...
    public List<UniMajorInfoZXAVO> findByIsBenSuo(Integer isBenSuo){
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findByIsBenSuo(isBenSuo);
        if(  inList != null){
            for (UniMajorInfoZXA inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXAVO.class));
            }
...
//    public Map<String, Object> listbyPage(String uniMajorCode,Integer pici,String schoolName,Integer schoolLvl1,Integer schoolLvl2,Integer schoolLvl3,String schoolType,String province,String majorName,Pageable pageable) {
        ...
        org.springframework.data.domain.Page<UniMajorInfoZXA> pageResult = null;
...
//        if (StringUtils.isEmpty(uniMajorCode)  && StringUtils.isEmpty(pici)  && StringUtils.isEmpty(schoolName)  && StringUtils.isEmpty(schoolLvl1)  && StringUtils.isEmpty(schoolLvl2)  && StringUtils.isEmpty(schoolLvl3)  && StringUtils.isEmpty(schoolType)  && StringUtils.isEmpty(province)  && StringUtils.isEmpty(majorName) ) {
            ...
            Specification<UniMajorInfoZXA> spec = (Specification<UniMajorInfoZXA>) (root, query, cb) -> {
...
//                        }
        ...
        List<UniMajorInfoZXAVO> list = BeanMapper.mapList(pageResult.getContent(), UniMajorInfoZXAVO.class);
    ...
    @Override
    public List<UniMajorInfoZXAVO> findByUniMajorCodeIn(List<String> ids)
    {
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findByUniMajorCodeIn( ids);
        if (inList != null) {
            for (UniMajorInfoZXA inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXAVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoZXAVO> findByUniMajorCodeAndExamYear(String uniMajorCode, String examYear) {
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findByUniMajorCodeAndExamYear(uniMajorCode, examYear);
        if (inList != null) {
            for (UniMajorInfoZXA inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXAVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoZXAVO> findByUniMajorCodeAndMajorCodeAndExamYear(String uniMajorCode, String majorCode, String examYear) {
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findByUniMajorCodeAndMajorCodeAndExamYear(uniMajorCode, majorCode, examYear);
        if (inList != null) {
            for (UniMajorInfoZXA inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXAVO.class));
            }
    ...
    @Override
    @Transactional
    public List<UniMajorInfoZXA> batchSave(List<UniMajorInfoZXA> entityList){
//        List<UniMajorInfoZXA> entityList = new ArrayList<>();
//        for (int i = 0; i < list.size(); i++) {
//            entityList.add(BeanMapper.map(list.get(i), UniMajorInfoZXA.class));
//        }
...
//            entityManager.persist(BeanMapper.map(list.get(i), UniMajorInfoZXA.class));
    ...
    @Override
    @Transactional
    public List<UniMajorInfoZXA> batchUpdate(List<UniMajorInfoZXA> list){
        for (int i = 0; i < list.size(); i++) {
            if(i>0 && i % ConstDefine.BATCH_SIZE == 0){
                entityManager.flush(); // flush 触发事务 同步
                entityManager.clear(); // 清除缓存;  清除实体
...
//        List<UniMajorInfoZXALatestVO> voList = new ArrayList<>();
    ...
    @Override
    public List<UniMajorInfoZXAVO> findByPiciAndExamYear(Integer pici, String examYear){
        List<UniMajorInfoZXAVO> voList = new ArrayList<>();
        List<UniMajorInfoZXA> inList = imp.findByPiciAndExamYear(pici, examYear);
        if (inList != null) {
            for (UniMajorInfoZXA inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXAVO.class));
            }
