...
@Transactional
@Service
public class UniMajorInfoZXBServiceImpl  implements UniMajorInfoZXBService {


    @Autowired
    private UniMajorInfoZXBRepo imp;
     ...
     */
    @Override
    public UniMajorInfoZXB saveUniMajorInfoZXB(UniMajorInfoZXBVO uniMajorInfoA) {

        return imp.save(BeanMapper.map(uniMajorInfoA, UniMajorInfoZXB.class));
    }
    /**
     * 删除单条记录
     * @param id
    ...
    public boolean deleteByKeyId(Long id) {
        UniMajorInfoZXB entity = imp.findById(id).orElse(null);
     ...
     */
    @Override
    public UniMajorInfoZXBVO findOneById(Long id) {
        UniMajorInfoZXB entity = imp.findById(id).orElse(null);
        if(entity == null) return null;
        return  BeanMapper.map(entity, UniMajorInfoZXBVO.class);
    }
    @Override
    public List<UniMajorInfoZXBVO> findByIdIn(List<Long> id) {
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findByIdIn(id);
        if(  inList != null){
            for (UniMajorInfoZXB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXBVO.class));
            }
        }
        return  voList;
    }    /**
     ...
     */
    @Override
    public List<UniMajorInfoZXBVO> getAllUniMajorInfoZXBs() {
        return BeanMapper.mapList(imp.findAll(), UniMajorInfoZXBVO.class);
    }
    /**
     * 更新记录
     ...
     */
    @Override
    public String updateUniMajorInfoZXB(UniMajorInfoZXBVO uniMajorInfoAVO) {
        UniMajorInfoZXB uniMajorInfoA = BeanMapper.map(uniMajorInfoAVO, UniMajorInfoZXB.class);
  ...
  public List<UniMajorInfoZXBVO> findByUniMajorCode(String uniMajorCode){
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findByUniMajorCode(uniMajorCode);
        if(  inList != null){
            for (UniMajorInfoZXB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXBVO.class));
            }
  ...
  public List<UniMajorInfoZXBVO> findByUniMajorCodeLike(String uniMajorCode){
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findByUniMajorCodeLike("%"+uniMajorCode+"%");
        if(  inList != null){
            for (UniMajorInfoZXB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXBVO.class));
            }
  ...
  public List<UniMajorInfoZXBVO> findByPici(Integer pici){
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findByPici(pici);
        if(  inList != null){
            for (UniMajorInfoZXB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXBVO.class));
            }
  ...
  public List<UniMajorInfoZXBVO> findBySchoolName(String schoolName){
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findBySchoolName(schoolName);
        if(  inList != null){
            for (UniMajorInfoZXB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXBVO.class));
            }
  ...
  public List<UniMajorInfoZXBVO> findBySchoolNameLike(String schoolName){
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findBySchoolNameLike("%"+schoolName+"%");
        if(  inList != null){
            for (UniMajorInfoZXB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXBVO.class));
            }
  ...
  public List<UniMajorInfoZXBVO> findBySchoolLvl1(Integer schoolLvl1){
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findBySchoolLvl1(schoolLvl1);
        if(  inList != null){
            for (UniMajorInfoZXB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXBVO.class));
            }
  ...
  public List<UniMajorInfoZXBVO> findBySchoolLvl2(Integer schoolLvl2){
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findBySchoolLvl2(schoolLvl2);
        if(  inList != null){
            for (UniMajorInfoZXB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXBVO.class));
            }
  ...
  public List<UniMajorInfoZXBVO> findBySchoolLvl3(Integer schoolLvl3){
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findBySchoolLvl3(schoolLvl3);
        if(  inList != null){
            for (UniMajorInfoZXB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXBVO.class));
            }
  ...
  public List<UniMajorInfoZXBVO> findBySchoolType(String schoolType){
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findBySchoolType(schoolType);
        if(  inList != null){
            for (UniMajorInfoZXB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXBVO.class));
            }
  ...
  public List<UniMajorInfoZXBVO> findBySchoolTypeLike(String schoolType){
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findBySchoolTypeLike("%"+schoolType+"%");
        if(  inList != null){
            for (UniMajorInfoZXB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXBVO.class));
            }
  ...
  public List<UniMajorInfoZXBVO> findByProvince(String province){
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findByProvince(province);
        if(  inList != null){
            for (UniMajorInfoZXB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXBVO.class));
            }
  ...
  public List<UniMajorInfoZXBVO> findByProvinceLike(String province){
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findByProvinceLike("%"+province+"%");
        if(  inList != null){
            for (UniMajorInfoZXB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXBVO.class));
            }
  ...
  public List<UniMajorInfoZXBVO> findByMajorName(String majorName){
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findByMajorName(majorName);
        if(  inList != null){
            for (UniMajorInfoZXB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXBVO.class));
            }
  ...
  public List<UniMajorInfoZXBVO> findByMajorNameLike(String majorName){
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findByMajorNameLike("%"+majorName+"%");
        if(  inList != null){
            for (UniMajorInfoZXB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXBVO.class));
            }
    ...
    public List<UniMajorInfoZXBVO> findByIsZhongWai(Integer isZhongWai){
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findByIsZhongWai(isZhongWai);
        if(  inList != null){
            for (UniMajorInfoZXB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXBVO.class));
            }
    ...
    public List<UniMajorInfoZXBVO> findByIsBenSuo(Integer isBenSuo){
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findByIsBenSuo(isBenSuo);
        if(  inList != null){
            for (UniMajorInfoZXB inf:inList) {
                voList.add(BeanMapper.map(inf,UniMajorInfoZXBVO.class));
            }
...
//    public Map<String, Object> listbyPage(String uniMajorCode,Integer pici,String schoolName,Integer schoolLvl1,Integer schoolLvl2,Integer schoolLvl3,String schoolType,String province,String majorName,Pageable pageable) {
        ...
        org.springframework.data.domain.Page<UniMajorInfoZXB> pageResult = null;
...
//        if (StringUtils.isEmpty(uniMajorCode)  && StringUtils.isEmpty(pici)  && StringUtils.isEmpty(schoolName)  && StringUtils.isEmpty(schoolLvl1)  && StringUtils.isEmpty(schoolLvl2)  && StringUtils.isEmpty(schoolLvl3)  && StringUtils.isEmpty(schoolType)  && StringUtils.isEmpty(province)  && StringUtils.isEmpty(majorName) ) {
            ...
            Specification<UniMajorInfoZXB> spec = (Specification<UniMajorInfoZXB>) (root, query, cb) -> {
...
//                        }
        ...
        List<UniMajorInfoZXBVO> list = BeanMapper.mapList(pageResult.getContent(), UniMajorInfoZXBVO.class);
    ...
    @Override
    public List<UniMajorInfoZXBVO> findByUniMajorCodeIn(List<String> ids)
    {
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findByUniMajorCodeIn( ids);
        if (inList != null) {
            for (UniMajorInfoZXB inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXBVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoZXBVO> findByUniMajorCodeAndExamYear(String uniMajorCode, String examYear) {
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findByUniMajorCodeAndExamYear(uniMajorCode, examYear);
        if (inList != null) {
            for (UniMajorInfoZXB inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXBVO.class));
            }
    ...
    @Override
    public List<UniMajorInfoZXBVO> findByUniMajorCodeAndMajorCodeAndExamYear(String uniMajorCode, String majorCode, String examYear) {
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findByUniMajorCodeAndMajorCodeAndExamYear(uniMajorCode, majorCode, examYear);
        if (inList != null) {
            for (UniMajorInfoZXB inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXBVO.class));
            }
    ...
    @Override
    @Transactional
    public List<UniMajorInfoZXB> batchSave(List<UniMajorInfoZXB> entityList){
//        List<UniMajorInfoZXB> entityList = new ArrayList<>();
//        for (int i = 0; i < list.size(); i++) {
//            entityList.add(BeanMapper.map(list.get(i), UniMajorInfoZXB.class));
//        }
...
//            entityManager.persist(BeanMapper.map(list.get(i), UniMajorInfoZXB.class));
    ...
    @Override
    @Transactional
    public List<UniMajorInfoZXB> batchUpdate(List<UniMajorInfoZXB> list){
        for (int i = 0; i < list.size(); i++) {
            if(i>0 && i % ConstDefine.BATCH_SIZE == 0){
                entityManager.flush(); // flush 触发事务 同步
                entityManager.clear(); // 清除缓存;  清除实体
...
//        List<UniMajorInfoZXBLatestVO> voList = new ArrayList<>();
    ...
    @Override
    public List<UniMajorInfoZXBVO> findByPiciAndExamYear(Integer pici, String examYear){
        List<UniMajorInfoZXBVO> voList = new ArrayList<>();
        List<UniMajorInfoZXB> inList = imp.findByPiciAndExamYear(pici, examYear);
        if (inList != null) {
            for (UniMajorInfoZXB inf : inList) {
                voList.add(BeanMapper.map(inf, UniMajorInfoZXBVO.class));
            }
