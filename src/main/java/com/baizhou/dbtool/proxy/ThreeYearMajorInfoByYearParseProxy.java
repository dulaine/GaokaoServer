...
import com.baizhou.core.model.vo.MajorInfoY2VO;
import com.baizhou.core.model.vo.MajorInfoY3VO;
import com.baizhou.core.model.vo.MajorInfoYAllVO;
...
public class ThreeYearMajorInfoByYearParseProxy implements IExcelParseProxy {
    @Override
...
//        int year = param.NewVersion; // 0: 最近第一个年   1: 2年  2:3年
        ...
        String examYear = param.Year;
        System.out.println("开始解析专业  ");
//        Thread.sleep(ConstDefine.InsertDBInterval);
        for (int i = 0; i < protoList.size(); i++) {
            MajorInfoYear proto = protoList.get(i);
            //保存
            MajorInfoYAllVO majorInfoVO = MajorInfoYAllVO.ConvertFromDTO(proto, examYear);
            majorInfoVO.setId(null);
            DBProxy.Getinstance().MajorInfoYAllService.saveMajorInfoYAll(majorInfoVO);

            System.out.println("保存 " + majorInfoVO.getMajorName());
            Thread.sleep(ConstDefine.InsertDBIntervalQuick);
        }

        System.out.println("解析一页专业信息");
    }
    ...
    private List<MajorInfoYear> ConvertToProtoList(HashMap<String, HashMap<String, ExcelRowData>> UniMajorDic,
        ...
        for (Map.Entry<String, HashMap<String, ExcelRowData>> uniMajors : UniMajorDic.entrySet()) {
            ...
            for (Map.Entry<String, ExcelRowData> major : majorDic.entrySet()) {
                ...
                MajorInfoYear.Builder builder = Convert(row, Pici);
    ...
    private MajorInfoYear.Builder Convert(ExcelRowData row, int pici) {

        MajorInfoYear.Builder info = MajorInfoYear.newBuilder();
